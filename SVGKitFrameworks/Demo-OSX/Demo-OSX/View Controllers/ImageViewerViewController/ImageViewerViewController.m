...
@implementation ImageViewerViewController
{
    ...
    MainSVGViewerType _mainViewerType;
}
...
- (void)viewDidLoad {
    ...
    [self _updateDocumentView];
}

- (NSView *)documentView
{
    return [_viewerScrollView documentView];
}
...
{
    @synchronized (self) {
        _mainViewerType = mainViewerType;
...
/*
    if ([self.documentView isKindOfClass:[SVGKLayeredImageView class]]) {
        ...
        if (self.imageFile != nil) {
            svgImage = [SVGKImage imageWithContentsOfFile:_imageFile.URL.path];
        }
        ...
        NSSize frameSize = NSMakeSize(svgImage.CALayerTree.frame.size.width, svgImage.CALayerTree.frame.size.height);
...
- (void)_updateDocumentView
{
    ...
    switch (self.mainViewerType) {

        case MainSVGViewerTypeLayered:
            
            if (![documentView isKindOfClass:[SVGKLayeredImageView class]])
                documentView = [[SVGKLayeredImageView alloc] initWithSVGKImage:self.svgImage];
            
            break;
            
        case MainSVGViewerTypeFast:
        default:
        
            if (![documentView isKindOfClass:[SVGKFastImageView class]])
                documentView = [[SVGKFastImageView alloc] initWithSVGKImage:self.svgImage];
    ...
    if (self.viewerScrollView.documentView != documentView)
        self.viewerScrollView.documentView = documentView;
}

- (SVGKImage *)_createDefaultImage
{
    ...
    SVGKImage* defaultImage = [SVGKImage imageWithSource:[SVGKSourceString sourceFromContentsOfString:svgStringDefaultContents]];
